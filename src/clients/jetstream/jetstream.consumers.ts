import type { Consumer, ConsumerInfo, ConsumerConfig, ConsumerUpdateConfig, PushConsumer } from '../../libs/nats-jetstream.js';
...
export function consumersModule(this: JetstreamClient) {
    ...
    const create = async (streamName: string, consumerName: string, config: Partial<ConsumerConfig>): Promise<ConsumerInfo> => {
        ...
        return await jetstreamManager.consumers.add(streamName, fullConfig);
    }
    ...
    const update = async (streamName: string, consumerName: string, config: Partial<ConsumerUpdateConfig>): Promise<ConsumerInfo> => {
        ...
        // Update the consumer
        return await jetstreamManager.consumers.update(streamName, consumerName, config);
    }
    ...
    const destroy = async (streamName: string, consumerName: string): Promise<boolean> => {
        ...
        // Delete the consumer
        return await jetstreamManager.consumers.delete(streamName, consumerName);
    }
