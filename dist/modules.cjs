...
function pack(input, debug = false) {
  ...
  const encoded = (0, import_msgpack.encode)(input);
  if (debug) {
    try {
...
function unpack(input, debug = false) {
  const start = performance.now();
  const decoded = (0, import_msgpack.decode)(input);
  if (debug) {
    const duration = performance.now() - start;
...
function compress(input, debug = false) {
  const start = performance.now();
  const compressed = (0, import_snappy.compressSync)(input);
  if (debug) {
    try {
      const duration = performance.now() - start;
...
function decompress(output, debug = false) {
  const start = performance.now();
  const decompressed = (0, import_snappy.uncompressSync)(output);
  if (debug) {
    const duration = performance.now() - start;
...
var Common = class {
  ...
  getHeader() {
    return (0, import_nats_core.headers)();
  }
...
var setConsumerConfig = /* @__PURE__ */ __name(({
  ...
  consumerDeliverPolicy = import_jetstream.DeliverPolicy.All,
